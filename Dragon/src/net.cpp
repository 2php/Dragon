	...
	for (int layer_id = layers.size()-1; layer_id >= 0; layer_id--){
		...
		if (Dragon::get_root_solver()||Dragon::get_arch()==Dragon::DEVICE){
			if (layer_need_backward[layer_id])
				LOG_IF(INFO, Dragon::get_root_solver()) << "Layer: " << layer_names[layer_id] << " need back-propogation.";
			else LOG_IF(INFO, Dragon::get_root_solver()) << "Layer: " << layer_names[layer_id] << " does not need back-propogation.";
		}
