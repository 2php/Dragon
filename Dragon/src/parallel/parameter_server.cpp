...
void ParameterServer<BlobType>::handle_message(int param_id, int node_id){
	...
	// LOG(INFO) << "[Server]: receive diff " << status.MPI_TAG << " from " << status.MPI_SOURCE;
	update(param_id);
	//	feedback updated-data 
	MPI_Send((void*)param->cpu_data(), param->count(), getType(Dtype(0.0)),
		node_id, param_id, MPI_COMM_WORLD);
	// LOG(INFO) << "[Server]: feedback data " << param_id << " to " << node_id;
...
void ParameterServer<BlobType>::do_listen(){
	while (1){
		...
		// LOG(INFO) << "[Server]: get request " << status.MPI_TAG << " from " << status.MPI_SOURCE;
...
void ParameterServer<BlobType>::update(int param_id){
	...
	MPI_Status status;
	boost::shared_ptr<BlobType> param = params[param_id];
	// LOG(INFO) << "[Public]: request update " << param_id << " from" << rank;
	...
	if (rank == 0){
		...
		// LOG(INFO) << "[Server]: finish update";
		update_cnt++;
		if (update_cnt % 100000 == 0)
			LOG(INFO) << "[Server]: reports:  " << update_cnt << " updates";
	}
	//	call by Net
	else{
