...
void DragonThread::initializeThread(int device, Dragon::Mode mode, Dragon::Arch arch,unsigned int rand_seed, bool root_solver){
	Dragon::set_device(device);
...
void DragonThread::startThread(){
	...
	unsigned int seed = Dragon::get_random_seed();
	bool root_solver = Dragon::get_root_solver();
	try{
		thread.reset(new boost::thread(&DragonThread::initializeThread,this, device, mode,arch,seed, root_solver));
