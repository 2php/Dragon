...
template <typename Dtype>
void MPIGatherLayer<Dtype>::forward_gpu(const vector<Blob<Dtype>*> &bottom, const vector<Blob<Dtype>*> &top){
#ifndef WITH_CUDA_AWARE
	forward_cpu(bottom, top);
#else
	Dtype* bottom_data = bottom[0]->mutable_gpu_data();
	int count = bottom[0]->count();
	if (this->comm_rank == this->comm_root){
		dragon_gpu_copy(count, top[this->comm_rank]->mutable_gpu_data(), bottom_data);
		for (int i = 0; i < this->comm_size; i++){
			if (i == this->comm_root) continue;
			if (typeid(Dtype) == typeid(double))
				MPI_Recv(top[i]->mutable_gpu_data(), count, MPI_DOUBLE, i, 0, this->comm, MPI_STATUS_IGNORE);
			else MPI_Recv(top[i]->mutable_gpu_data(), count, MPI_FLOAT, i, 0, this->comm, MPI_STATUS_IGNORE);
		}
	}else{
		if (typeid(Dtype) == typeid(double))
			MPI_Send(bottom_data, count, MPI_DOUBLE, this->comm_root, 0, this->comm);
...
template <typename Dtype>
void MPIGatherLayer<Dtype>::backward_gpu(const vector<Blob<Dtype>*> &top,
	const vector<bool> &data_need_bp, const vector<Blob<Dtype>*> &bottom){
#ifndef WITH_CUDA_AWARE
	...
	int count = bottom[0]->count();
	if (this->comm_rank == this->comm_root){
		dragon_gpu_copy(count, bottom_diff, top[this->comm_rank]->mutable_gpu_diff());
		for (int i = 0; i < this->comm_size; i++){
			if (i == this->comm_root) continue;
			if (typeid(Dtype) == typeid(double))
				MPI_Send(top[i]->mutable_gpu_diff(), count, MPI_DOUBLE, i, 0, this->comm);
			else MPI_Send(top[i]->mutable_gpu_diff(), count, MPI_FLOAT, i, 0, this->comm);
		}
	}else{
		if (typeid(Dtype) == typeid(double))
			MPI_Recv(bottom_diff, count, MPI_DOUBLE, this->comm_root, 0, this->comm, MPI_STATUS_IGNORE);
