...
template <typename Dtype>
void MPIBroadcastLayer<Dtype>::forward_gpu(const vector<Blob<Dtype>*> &bottom, const vector<Blob<Dtype>*> &top){
	...
	if (this->comm_rank == this->comm_root){
		...
		dragon_gpu_copy(count, top[0]->mutable_gpu_data(), bottom_data);
	}
	//	sub processors
	else{
...
template <typename Dtype>
void MPIBroadcastLayer<Dtype>::backward_gpu(const vector<Blob<Dtype>*> &top,
	const vector<bool> &data_need_bp, const vector<Blob<Dtype>*> &bottom){
#ifndef WITH_CUDA_AWARE
	...
	if (this->comm_rank == this->comm_root){
		...
		for (int i = 0; i < this->comm_size; i++){
			...
			dragon_gpu_add(count, bottom_diff, top_diff, bottom_diff);
		}
	}else{
		Dtype* top_diff = top[0]->mutable_gpu_diff();
		int count = top[0]->count();
