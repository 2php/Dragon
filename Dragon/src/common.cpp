...
Dragon::Dragon():
	mode(Dragon::CPU), arch(Dragon::Arch::NORMAL),root_solver(true) {}
Dragon::~Dragon() { }
...
Dragon::Dragon() :
	mode(Dragon::CPU),arch(Dragon::NORMAL),root_solver(true),random_seed(3),
...
void Dragon::set_device(const int device_id) {
	...
	//	restore the seed
	set_random_seed(Get().random_seed);
}
#endif
